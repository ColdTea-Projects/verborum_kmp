@file:OptIn(ExperimentalStdlibApi::class, kotlin.js.ExperimentalWasmJsInterop::class)
...
internal actual fun browserOrigin(): String = js("location.origin")

internal actual fun browserSearch(): String = js("location.search")
...
private fun randomHex(size: Int): String =
    js("Array.from(crypto.getRandomValues(new Uint8Array(size))).map(function (b) { return b.toString(16).padStart(2, '0'); }).join('')")

internal actual fun nowMillis(): Double = js("Date.now()")
